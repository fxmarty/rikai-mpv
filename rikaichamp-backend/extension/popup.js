...
import { CopyKeys, CopyKanjiKeyStrings, CopyNextKeyStrings, } from './copy-keys.js';
import { groupSenses } from './grouping.js';
import { getHash } from './hash.js';
import { isSvgDoc, SVG_NS } from './svg.js';
import { getEraInfo } from './years.js';
import fs from 'fs';
...
const { JSDOM } = jsdom;
export function renderPopup(result, options) {
    ...
    switch (result.type) {
        case 'names':
            ...
            break;
        default:
            windowElem.append(renderWordEntries(result.data, result.names, result.moreNames, result.title, result.more, options, doc));
            break;
    }
...
function getDefaultContainer(doc) {
...
function resetContainer(container, { document: doc, popupStyle }) {
...
function renderWordEntries(entries, names, moreNames, title, more, options, doc) {
    var _a;
    const container = doc.createElement('div');
    ...
    for (const entry of entries) {
        ...
        if (index === selectedIndex) {
            entryDiv.classList.add(options.copyState === 1 /* Active */ ? '-selected' : '-flash');
        }
        index++;
        ...
        if ((_a = entry.romaji) === null || _a === void 0 ? void 0 : _a.length) {
    ...
    const copyDetails = renderCopyDetails(options.copyNextKey, doc, options.copyState, typeof options.copyType !== 'undefined' ? options.copyType : undefined);
    if (copyDetails) {
        container.append(copyDetails);
    }
    return container;
}
function renderEraInfo(meta, eraInfo, doc) {
...
function renderBonusNames(names, doc, moreNames) {
    const container = doc.createElement('div');
...
function renderKana(kana, options, doc) {
    const accents = kana.a;
    if (options.accentDisplay === 'none' ||
    ...
    if (options.accentDisplay === 'downstep') {
        if (!accentPos) {
            ...
            return (moraSubstring(kana.ent, 0, accentPos) +
                'ꜜ' +
...
function appendHeadwordInfo(info, parent, doc) {
    if (!info || !info.length) {
        return;
    }
    for (const i of info) {
...
function appendPriorityMark(priority, parent, doc) {
...
function renderDefinitions(entry, options, doc) {
...
function renderSense(sense, options, doc) {
    ...
    if (options.posDisplay !== 'none') {
        for (const pos of sense.pos || []) {
            ...
            switch (options.posDisplay) {
                case 'expl':
                    posSpan.lang = getLangTag();
                    posSpan.append(i18next.t(`pos_label_${pos.replace(/-/g, '_')}` + '.message') ||
                        pos);
                    break;
                case 'code':
                    posSpan.append(pos);
...
function appendGlosses(glosses, parent, doc) {
...
function renderLangSources(sources, doc) {
...
function renderNamesEntries(entries, more, options, doc) {
    const container = doc.createElement('div');
    const titleDiv = doc.createElement('div');
    container.append(titleDiv);
    titleDiv.classList.add('title');
...
function renderNameTranslation(tr, doc) {
...
function getSelectedIndex(options, numEntries) {
    return typeof options.copyState !== 'undefined' &&
        options.copyState !== 0 /* Inactive */ &&
        typeof options.copyIndex !== 'undefined' &&
        numEntries
        ? options.copyIndex % numEntries
        : -1;
}
function renderCopyDetails(copyNextKey, doc, copyState, copyType, options = { kanji: false }) {
    if (typeof copyState === 'undefined' || copyState === 0 /* Inactive */) {
        return null;
    }
    ...
    if (copyState === 2 /* Finished */ && typeof copyType !== 'undefined') {
        copyDiv.classList.add('-finished');
        copyDiv.append(renderCopyStatus(getCopiedString(copyType), doc));
    }
    else if (copyState === 3 /* Error */) {
        copyDiv.classList.add('-error');
        copyDiv.append(renderCopyStatus(i18next.t('content_copy_error' + '.message'), doc));
    }
    return copyDiv;
}
...
function renderCopyStatus(message, doc) {

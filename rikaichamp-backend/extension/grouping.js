export function groupSenses(senses) {
    ...
    for (const sense of senses) {
        ...
        //    also has no POS.
        if ((previousPos && sense.pos && sense.pos.includes(previousPos)) ||
    ...
    for (const group of groups) {
        ...
        if (commonMisc.length) {
            ...
            group.senses = group.senses.map((sense) => {
                var _a;
                return (Object.assign(Object.assign({}, sense), { misc: (_a = sense.misc) === null || _a === void 0 ? void 0 : _a.filter((misc) => !commonMisc.includes(misc)) }));
            });
        }
    }
    return groups;
...
function dropPos(sense, posToDrop) {
    let pos = sense.pos
        ? sense.pos.filter((pos) => Array.isArray(posToDrop) ? !posToDrop.includes(pos) : pos !== posToDrop)
        : undefined;
    if (pos && !pos.length) {
        pos = undefined;
    }
    return Object.assign(Object.assign({}, sense), { pos });
}
